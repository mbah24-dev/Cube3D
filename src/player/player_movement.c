...
int	update_player_movement(t_engine *engine)
{
	...
	if (engine->player.move_y == 1)
		moved += move_player_forward(engine);
	if (engine->player.move_y == -1)
		moved += move_player_backward(engine);
	if (engine->player.move_x == -1)
		moved += strafe_player_left(engine);
	if (engine->player.move_x == 1)
		moved += strafe_player_right(engine);
	if (engine->player.rotate != 0)
		moved += handle_player_rotation(engine, engine->player.rotate);
	return (moved);
}
